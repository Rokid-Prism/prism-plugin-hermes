...
const protocol = field("protocol_version");
...
if (!id || !version || !icon || !svg || !protocol || !minHub || !command) throw new Error("manifest must define id, version, protocol, minimum Hub version, both icons, and command");
...
if (protocol !== "4" || !/^\d+\.\d+\.\d+$/.test(minHub)) throw new Error("manifest must declare PluginBridge protocol 4 and a stable minimum Hub version");
if (!nodeRange || !command.includes("${runtime.node}")) throw new Error("Node plugins must declare the shared Node runtime");
...
console.log(JSON.stringify({ id, version, protocol, min_hub_version: minHub, platform: process.env.PRISM_PLATFORM }));
